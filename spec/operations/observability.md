...
- v2 turn-network events: `v2_open` records commit-scope open latency/status and transfer mode; `v2_envelope` records envelope latency/status, idempotency freshness, reply class, fanout, and full-save use; `shadow_commit_accepted` and `shadow_commit_rejected` split commit outcomes into tail-queryable counters so operators can answer "is v2 healthy?" without inferring from generic request or cross-host RPC traffic. `v2_host_apply_fanout` records the post-accept write-through from an accepted transcript to routed object hosts, including touched-object count, host count, latency, and retryable failures.
